...
- **развернуть за вечер** на Netlify, GitHub Pages или своём VPS;
...
Это **не коммерческий SaaS**. Рекомендуем **свой деплой** (например на [Netlify](docs/USER-GUIDE.md)) и свой проект Supabase — тогда данные и инстанс полностью ваши.
...
### Повторение (SRS)
- Алгоритмы: **SM-2**, **FSRS** (с настройкой **желаемого удержания** и выравниванием нагрузки *fuzz*), **коробки Лейтнера** — прогресс каждого хранится отдельно
...
- **Журнал повторений** и отдельный **экран статистики**: измеренное удержание, повторения за 30 дней, **прогноз нагрузки**, разбивка по папкам
...
| Cloudflare Pages (логин, KV, деплой) | [docs/cloudflare-pages-setup.md](docs/cloudflare-pages-setup.md) |
...
| Netlify Functions / Cloudflare Pages, миграции SQL | [DEPLOY.md](./DEPLOY.md) · [cloudflare-pages-setup.md](./cloudflare-pages-setup.md) |
...
| SRS | SM-2, FSRS (`ts-fsrs`, желаемое удержание + fuzz), Leitner |
| Serverless | Cloudflare Pages Functions (`functions/api/`) для YouTube/LLM; Netlify (`netlify/functions/`) — запасной |
...
functions/api/          — Cloudflare Pages Functions (YouTube + LLM)
netlify/functions/      — запасной Netlify-прокси (до полной проверки CF)
wrangler.toml           — Cloudflare Pages + KV YT_JOBS
tests/                  — unit-тесты
```
...
**KAR-dots** is a **self-hosted, open-source** flashcard PWA for personal use. Deploy it on Netlify, GitHub Pages, or your own server — share the link with friends; each person gets a private collection (optional Supabase auth). Not a commercial SaaS: you own the instance and the data.
