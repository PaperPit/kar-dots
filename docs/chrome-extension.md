...
Расширение добавляет кнопку **«Карточки»** в правом верхнем углу YouTube. По нажатию поверх ролика открывается панель-оверлей (страница не сдвигается, новых вкладок нет) с теми же настройками режима, что в диалоге приложения (Слова / Фразы / Слова+фразы / Предложения), превью с галочками и сохранением в облачный аккаунт на [kar-tochki.pages.dev](https://kar-tochki.pages.dev).
...
6. Откройте любое видео на YouTube → нажмите **Карточки** → панель поверх ролика → выберите режим и папку → **Сформировать** → отметьте карточки → **Создать**.

Версия на карточке расширения должна быть **не ниже 1.0.6**. Красные подчёркивания `chrome.*` в редакторе — это только типы IDE, не ошибка рантайма.

Если панель пустая: `chrome://extensions` → **Errors** у расширения; при необходимости Remove → Load unpacked заново.
...
В панели → **Отключить** (чистит сессию в `chrome.storage`). Либо удалите расширение в `chrome://extensions`.
