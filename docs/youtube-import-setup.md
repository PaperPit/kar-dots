...
Ключ Supadata **обязателен у каждого пользователя** (Настройки → «Карточки из YouTube» → «Настроить»).  
Для генерации карточек нужен **свой** Gemini и/или Groq — серверные ключи не подставляются.
...
| Ключ | Где | Обязателен |
|---|---|---|
| Supadata | Настройки → «Карточки из YouTube» | да (транскрипт) |
| Gemini | там же | да, хотя бы один из Gemini/Groq |
| Groq | там же | да, хотя бы один из Gemini/Groq |

Ключи передаются на сервер приложения только при импорте и хранятся в настройках пользователя (синхронизируются с аккаунтом). Серверные `*_API_KEY` в Cloudflare / `.env` **не используются** как fallback — иначе анонимный `POST /api/yt-generate` мог бы тратить чужую квоту.

Опционально на сервере можно задать только имена моделей: `GEMINI_MODEL`, `GROQ_MODEL` (не секреты биллинга).
...
Ключи — только из настроек приложения (Настройки → «Карточки из YouTube»). Для длинных транскриптов состояние джоба — in-memory (dev) или Workers KV (прод).
