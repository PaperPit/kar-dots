...
(() => {
  ...
  function renderForm(error = "") {
    ...
    for (const mo2 of MODES) {
      modeSeg.append(
        el(
          "button",
          {
            type: "button",
            class: mo2.id === mode ? "active" : "",
            onclick: () => {
              if (mode === mo2.id) return;
              mode = mo2.id;
  ...
  function videoTitle2() {
    const el2 = document.querySelector("h1.ytd-watch-metadata yt-formatted-string") || document.querySelector("h1 yt-formatted-string") || document.querySelector("h1.ytd-video-primary-info-renderer") || document.querySelector("#title h1");
    return (el2?.textContent || document.title || "").replace(/ - YouTube$/, "").trim();
  }
  ...
  function ensureOverlay() {
    ...
    if (host?.shadowRoot) {
      const app2 = host.shadowRoot.querySelector(".kar-ext-app");
      return { root: host, app: app2 };
    }
    ...
    <div class="wrap">
      ...
      <aside class="panel" role="dialog" aria-label="\u041A\u0410\u0420-\u0442\u043E\u0447\u043A\u0438">
        <header class="bar">
          ...
          <button type="button" class="bar-close" data-kar-close="1" aria-label="\u0417\u0430\u043A\u0440\u044B\u0442\u044C">\xD7</button>
        </header>
  ...
  `;
    ...
    const app = shadow.querySelector(".kar-ext-app");
    return { root: host, app };
  }
  async function ensurePanelMounted(app) {
    if (panelMounted) {
      mountKarPanel(app);
      return;
    }
  ...
  async function showOverlay() {
    const { root: root2, app } = ensureOverlay();
    await ensurePanelMounted(app);
    root2.style.display = "block";
    root2.setAttribute("aria-hidden", "false");
  }
