// Cloudflare Pages Function: генерация карточек из транскрипта. Основной провайдер — Gemini;
...
async function runGemini(env, apiKey, prompt) {
...
async function callGroqWithFallback(env, apiKey, prompt) {
...
async function handler(req, env) {
  ...
  if (mode === 'sentences') {
    ...
    if (geminiKey) {
      const gem = await runGemini(env, geminiKey, prompt);
    ...
    if (text === null && groqKey) {
      const groq = await callGroqWithFallback(env, groqKey, prompt);
