...
export class MiniSupabase {
  ...
  async _fetch(url: string, opts: RequestInit = {}, timeoutMs = 12000): Promise<Response> {
    ...
    try {
      ...
      if (e instanceof Error && e.name === "AbortError") {
        throw new TimeoutError("Превышено время ожидания сети")
      }
      throw e
    } finally {
      clearTimeout(t)
  ...
  async signUp(email: string, password: string): Promise<{ session: AuthSession | null; needConfirm: boolean }> {
    ...
    if (data.access_token) {
      ...
      return { session: this.session, needConfirm: false }
    }
  ...
  async signOut() {
    try {
      await this._fetch(this.url + "/auth/v1/logout", {
        method: "POST",
  ...
  userId(): string | null {
    return this.session && this.session.user ? (this.session.user.id ?? null) : null
  }

  /** Публичный доступ к текущей сессии (user.email и т.п.). */
  ...
  async count(table: string, query: string): Promise<number> {
    ...
    if (!r.ok) {
      await handle(r) // бросит RequestError с текстом ошибки
      return 0
    }
  ...
  async remove(table: string, filter: string): Promise<unknown> {
    await this.ensureFresh()
    const r = await this._fetch(this.url + "/rest/v1/" + table + "?" + filter, {
      method: "DELETE",
      headers: this._authHeaders()
...
async function handle(r: Response): Promise<unknown> {
  ...
  const data = await r.json().catch(() => ({}))
  const msg =
    data.message || data.error_description || data.error || "Ошибка запроса (" + r.status + ")"
  const err = new RequestError(msg)
  err.status = r.status
  throw err
}

function withExpiry(data: AuthResponse): AuthSession {
