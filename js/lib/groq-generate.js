 ...
 * @returns {string[]}
 */
export function groqModelsToTry(envModel) {
...
export function isGroqModelBlocked(raw) {
  return /blocked at the project level|enable this model in the project settings/i.test(String(raw || ''));
}

export function isGroqModelUnavailable(raw) {
  const msg = String(raw || '').toLowerCase();
  return isGroqModelBlocked(raw)
...
export function shouldTryNextGroqModel(status, rawMessage) {
...
export function formatGroqGenerateError(raw) {
