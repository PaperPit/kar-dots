...
export function animateModalIn(overlay, box) {
  if (!motionEnabled()) {
...
export function animateModalOut(overlay, box) {
  if (!motionEnabled()) {
...
export function animateToastIn(t) {
  if (!motionEnabled()) {
...
export function animateToastOut(t) {
  if (!motionEnabled()) {
...
export function animateBootSplashOut(splash) {
  if (!splash) return Promise.resolve();
  if (!motionEnabled()) {
    splash.remove();
    return Promise.resolve();
  }
  return run(splash, { opacity: [1, 0], scale: [1, 0.96] }, { duration: 0.35, ease: EASE })
    .then(() => splash.remove());
}
