...
export function normalizeOrpheusVoice(id) {
  const v = String(id || '').trim().toLowerCase();
  return VOICE_IDS.has(v) ? v : 'hannah';
}

export function orpheusVoiceLabel(id) {
...
export function orpheusEnabled(settings) {
  return settings?.ttsOrpheus === true;
}

export function truncateForOrpheus(text) {
...
export function formatOrpheusError(raw) {
...
export function isOrpheusTermsError(raw) {
