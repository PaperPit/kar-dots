...
export function renderAuth(busyMsg?: string) {
  ...
  content.append(localBtn, el("p", { class: "auth-note" }, t("auth.demoNote")))

  // Legacy Supabase cloud — вторичный блок (фаза 1: deprecated в UI).
  const email = el(
    "input",
    {
      class: "input",
  ...
  const btnIn = el("button", { class: "btn block" }, t("auth.signIn")) as HTMLButtonElement
  ...
  async function doAuth(signup: boolean) {
    if (!email.value.trim() || pass.value.length < 6) {
      toast(t("auth.needCredentials"), "error")
      return
    }
    btnIn.disabled = true
    try {
  ...
  const cloudBody = cloudConfigured
    ? [
        el("p", { class: "auth-note auth-legacy-note" }, t("auth.cloudLegacyHint")),
        el("div", { class: "field" }, email),
        el("div", { class: "field" }, pass),
        btnIn,
        el("p", { class: "auth-note" }, [t("auth.noAccount"), btnUp])
      ]
    : [el("p", { class: "modal-text modal-text-flush" }, t("auth.cloudNotConfigured"))]

  const cloudDetails = el("details", { class: "auth-legacy" }, [
    el("summary", { class: "auth-legacy-summary" }, t("auth.cloudLegacySummary")),
    el("div", { class: "auth-card auth-legacy-card" }, cloudBody)
  ])

  content.append(el("div", { class: "auth-or" }, "· · ·"), cloudDetails)
  app.append(el("main", { class: "main" }, content))
  requestAnimationFrame(() => animateFadeIn(content))
}
