...
export function openCardPreview(ctx: PreviewCtx) {
  ...
  const pm: ModalHandle = modal(
    el("div", null, [
      el("h3", { class: "modal-title", id: titleId }, t("cardEditor.preview.title")),
      el("p", { class: "modal-text muted card-preview-lead" }, t("cardEditor.preview.lead")),
      wrap,
      el("div", { class: "modal-actions modal-actions-center" }, [
        el(
          "button",
          { type: "button", class: "btn primary", onclick: () => pm.close() },
          t("cardEditor.preview.close")
        )
      ])
    ]),
