...
import { shakyCardIds } from "../../lib/shaky.js"
...
function matchesSearch(card: SrsCard, query: string) {
  ...
  const hay = [stripHtml(card.front), stripHtml(card.back), stripHtml(card.description || "")]
    .join(" ")
    .toLowerCase()
...
export async function renderFolder(folderId: string) {
  ...
  const now = Date.now()
  // Журнал — не критичный источник: нет его, значит просто нет пометок.
  const shakyIds = await getAllReviews()
    .then(shakyCardIds)
      ...
      "button",
      {
        ...
        onclick: async () => {
          const yes = await confirmDialog(
            isPack
              ? t("folder.screen.confirm.deletePackTitle")
              : t("folder.screen.confirm.deleteFolderTitle"),
            isPack
            ...
            isPack ? t("folder.screen.confirm.deletePackOk") : t("common.delete"),
            true,
            crowTombIcon()
          )
          if (!yes) return
          ...
          toast(
            isPack ? t("folder.screen.toast.packDeleted") : t("folder.screen.toast.folderDeleted")
          )
          nav("#home")
        }
      },
  ...
  const reviewBtn =
    due > 0
      ? (el(
          ...
          [svgNode(ICONS.play), t("folder.screen.reviewDue", { n: due })]
        ) as HTMLButtonElement)
      : null

  const addRow = el("div", { class: "folder-actions-pair" }, [
    el("button", { class: "btn", onclick: () => cardDialog(folderId, undefined) }, [
      svgNode(ICONS.plus),
      t("folder.screen.addCard")
    ]),
    el("button", { class: "btn", onclick: () => bulkCardDialog(folderId) }, [
      svgNode(ICONS.plus),
      t("folder.screen.addBulk")
    ])
  ])
  ...
  const ytBtn = isPack
    ...
    : (el(
        ...
        [svgNode(ICONS.youtube), t("settings.yt.title")]
      ) as HTMLButtonElement)

  const cramBtn = cards.length
    ? (el(
        "button",
        {
        ...
        [svgNode(ICONS.play), t("folder.screen.cramAll")]
      ) as HTMLButtonElement)
    : null
  ...
  const actions = el(
    ...
    [reviewBtn, addRow, ytBtn, cramBtn].filter(Boolean)
  )
  ...
  const filterAllBtn = el(
    ...
    t("folder.screen.filterAll")
  ) as HTMLButtonElement
  const filterDueBtn = el(
    "button",
    { type: "button" },
    t("folder.screen.filterDue")
  ) as HTMLButtonElement
  filterSeg.append(filterAllBtn, filterDueBtn)

  const toolbar = el("div", { class: "folder-toolbar" }, [searchInput, filterSeg])

  const listMount = el("div", { class: "card-list" }, [])
  const emptyFilter = el(
    "p",
    { class: "folder-filter-empty muted hidden" },
  ...
  function updateEmptyState(shown: number) {
    ...
    if (shown === 0 && cards.length && (searchInput.value.trim() || filterMode === "due")) {
      emptyFilter.textContent =
        filterMode === "due" && !searchInput.value.trim()
  ...
  function cardRow(c: SrsCard, i: number, algoName: Algo, virtual: boolean) {
    ...
    let chip
    if (SRS.isNew(c, algoName))
      chip = el("span", { class: "srs-chip new" }, t("folder.screen.chipNew"))
    else if (SRS.isDue(c, algoName, now))
      chip = el("span", { class: "srs-chip due" }, t("folder.screen.chipDue"))
    else {
      const d = SRS.dueOf(c, algoName)
      chip = el(
        "span",
        { class: "srs-chip" },
        t("folder.screen.chipIn", {
          when: SRS.fmtDays(Math.max(1, Math.round(((d ?? Date.now()) - Date.now()) / 86400000)))
        })
      )
    }
    // «Шаткая» — по журналу: чаще проваливалась и отвечалась медленнее
    // собственной нормы. Планировщик этого ещё не видит, а пользователь
    // уже может заметить и переписать формулировку.
    const shakyChip = shakyIds.has(c.id ?? "")
      ? el(
          "span",
          { class: "srs-chip shaky", title: t("folder.screen.chipShakyHint") },
          t("folder.screen.chipShaky")
        )
      : null
        ...
        el(
          "button",
          {
            ...
            onclick: async (e) => {
              e.stopPropagation()
              const yes = await confirmDialog(
                t("folder.screen.confirm.deleteCardTitle"),
                textPreview(c),
                t("common.delete"),
                true,
                crowTombIcon()
