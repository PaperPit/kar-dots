...
import {
  ...
  importFromCaptionFile,
  generateYoutubeCards,
  createYoutubeCardsBatch,
...
export function youtubeImportDialog(folderId: string) {
  ...
  function renderForm(prefill: YTPrefill = {}) {
    ...
    const filePickBtn = el(
      ...
      t("folder.yt.pickFile")
    ) as HTMLButtonElement
    const filePicker = el("div", { class: "yt-file-picker" }, [fileInput, filePickBtn, fileNameEl])

    const fileUrlInput = el(
      "input",
      {
        class: "input",
        type: "url",
    ...
    const filePanel = el(
      "div",
      { class: "yt-source-panel" + (source === "file" ? "" : " hidden") },
      [
    ...
    const sentencesOpts = el(
      "div",
      { class: "yt-sentences-options" + (mode === "sentences" ? "" : " hidden") },
      [
    ...
    const errEl = el(
      "p",
      { class: "yt-error" + (prefill.error ? "" : " hidden") },
      prefill.error || ""
    )
  ...
  function renderProgress(text: string, hint: string = "") {
    ...
    const hintEl = hint ? el("p", { class: "yt-source-hint muted" }, hint) : null
    body.append(
    ...
    try {
      ...
      if (mode === "sentences") {
        ...
        renderPreview(
          ...
          [],
          dropped,
          { ...prefill, mode, mergeCues, transcriptSource: src },
          sentences,
          gen.truncated
        )
      } else {
        ...
        renderPreview(video, phrases, words, dropped, {
          ...prefill,
          mode,
          mergeCues,
  ...
  function renderPreview(
    ...
    dropped: number,
    prefill: YTPrefill,
    sentences: YtCandidate[] = [],
    truncated: { total: number; used: number } | null = null
  ) {
    body.innerHTML = ""
    const items: YTItem[] = []
    ...
    if (!phrases.length && !words.length && !sentences.length) {
      const emptyMsg =
        prefill.mode === "sentences" ? t("folder.yt.empty.sentences") : t("folder.yt.empty.lexicon")
      body.append(
        el("p", { class: "modal-text" }, emptyMsg),
        el("div", { class: "modal-actions" }, [
          el(
            "button",
            { class: "btn ghost", onclick: () => renderForm(prefill) },
            t("folder.yt.otherVideo")
          ),
    ...
    function group(title: string, cands: YtCandidate[]) {
      ...
      const allChk = el(
        "input",
        { type: "checkbox", class: "chk", checked: true },
        []
      ) as HTMLInputElement
      const rows: YTRow[] = cands.map((c: YtCandidate) => {
        const chk = el(
          "input",
          {
            ...
            checked: true,
            onchange: () => {
              allChk.checked = cands.every(
                (_: unknown, i: number) => rowsItems[i]!.checkbox.checked
              )
              refreshAddBtn()
            }
          },
        ...
        const backInput = el(
          "input",
          { class: "input yt-back-input", value: c.back },
          []
        ) as HTMLInputElement
        const chipText =
          c.kind === "sentence"
          ...
          el("div", { class: "yt-row-main" }, [
            el("div", { class: "yt-row-top" }, [
              ...
              c.t !== null && c.t !== undefined
                ? el("span", { class: "yt-chip yt-chip-time" }, "▶ " + fmtTimestamp(c.t))
                : null
            ]),
            backInput
          ])
      ...
      allChk.addEventListener("change", () => {
        rowsItems.forEach((it: YTItem) => {
          it.checkbox.checked = allChk.checked
        })
        refreshAddBtn()
      })
      return el("div", { class: "yt-group" }, [
        el("label", { class: "yt-group-title" }, [
          allChk,
          el("span", null, t("folder.yt.groupTitle", { title, n: cands.length }))
        ]),
        el(
          "div",
          { class: "yt-rows" },
          rows.map((r: YTRow) => r.row)
        )
      ])
    }
      ...
      "button",
      {
        ...
        onclick: async () => {
          const selected = items
            .filter((it: YTItem) => it.checkbox.checked)
            .map((it) => ({
              cand: it.cand,
          ...
          const { ok, failed } = await createYoutubeCardsBatch(
            (card: { folder_id: string; front: string; back: string; description: string }) =>
              store.createCard(card as never),
            folderId,
            selected,
            video?.videoId ?? null
          )
    ...
    const sourceHintEl = hintText ? el("p", { class: "yt-source-hint muted" }, hintText) : null

    const droppedMsg =
      dropped > 0
        ? el(
            "p",
            { class: "yt-dropped muted" },
            prefill.mode === "sentences"
    ...
    const truncatedMsg =
      truncated && truncated.total > truncated.used
        ? el(
            "p",
            { class: "yt-dropped muted" },
