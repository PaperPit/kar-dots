...
export function boxDialog(box: Box | null) {
  ...
  function paintFolderPick() {
    ...
    const candidates = store.folders.filter(
      (f: Folder) => !f.box_id || (box && f.box_id === box.id)
    )
    ...
    for (const f of candidates) {
      ...
      folderList.append(
        el("label", { class: "box-folder-pick-row", for: id }, [
          chk,
  ...
  const m: ReturnType<typeof modal> = modal(
    el("div", null, [
      el(
        "h3",
        { class: "modal-title", id: titleId },
        box ? t("box.dialog.titleEdit") : t("box.dialog.titleNew")
      ),
      ...
      el("div", { class: "modal-actions" }, [
        el(
          "button",
          { type: "button", class: "btn ghost", onclick: () => m.close() },
          t("common.cancel")
        ),
        save
      ])
