...
export function runReviewSession(ctx: ReviewSessionContext) {
  ...
  function clearStage() {
    if (ctx.currentDestroy) {
      ctx.currentDestroy()
      ctx.currentDestroy = null
    }
    ...
    while (ctx.queue.length && !cardWorksInMode(ctx.queue[0]!, side)) {
      const msg =
        ctx.mode === "cloze"
          ...
          : side === "front"
            ? t("review.session.skipNoBack")
            : t("review.session.skipNoFront")
      toast(msg, "error")
      ctx.queue.shift()
    }
  }
  ...
  function recordFirstTryResult(
    card: SrsCard,
    { success, firstTry }: { success: boolean; firstTry: boolean }
  ) {
  ...
  function showStudyCard(first: boolean, forceMode?: ReviewMode) {
    ...
    if (activeMode === "type") {
      ...
      widget = createTypeModeCard(card, {
        promptSide,
        onSuccess,
        onFail,
      ...
      widget = createClozeModeCard(card, {
        promptSide,
        onSuccess,
        onFail,
        getSettings: () => store.settings
      ...
      widget = createVoiceModeCard(card, {
        promptSide,
        onSuccess,
        onFail,
        getSettings: () => store.settings
  ...
  function showFlipCard(card: SrsCard, first: boolean, promptSide: "front" | "back") {
    ...
    const { box, flip, swipeWrap, grades, getVisibleSide, destroy } = createFlipModeCard(card, {
      ...
      onFlip: (flipSide) => {
        if (store.settings.tts !== false && store.settings.ttsAuto)
          void speakCardSide(card, flipSide as "front" | "back")
      },
      ...
      onGradeDir: (dir) =>
        submitGrade(ctx, card, gradePayload(ctx.algo, dir === "right"), dir, { flipGrade: true })
    })
    ctx.speakBtn.onclick = async () => {
  ...
  function showMatchRound(
    first: boolean,
    {
      ...
      countAsOne = false
    }: { batchSize?: number; countAsOne?: boolean } = {}
  ) {
    skipUncheckableFromHead()
    if (!ctx.queue.length) {
      finish()
  ...
  function finish() {
    ...
    const stars = computeLessonStars({
      stats: ctx.stats as unknown as LessonStats,
      sessionCards: ctx.sessionTotal
    })
    ...
    const homeLabel = ctx.noteId
      ...
      : ctx.folderId
        ? t("review.empty.toFolder")
        : t("review.empty.toHome")
    ctx.stage.append(
      el("div", { class: "review-done" }, [
        el("img", {
          class: "review-done-raven",
          src: "icons/raven.svg",
          alt: "",
          draggable: "false"
        }),
