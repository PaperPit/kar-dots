...
function buildSpeechVoiceRow(s: SettingsLike, save: () => void, ttsEnabled: boolean) {
  const ruSelect = el(
    "select",
    { class: "input speech-voice-select", disabled: !ttsEnabled },
    []
...
export function buildAlgoGroup(s: SettingsLike, save: () => void) {
  ...
  const algoGroup = el("div", { class: "settings-group" }, [
    ...
    el("div", { class: "setting-row" }, [
      el("div", { class: "lab" }, [el("b", null, t("settings.algo.ttsAuto")), ttsAutoHint]),
  ...
  if (s.algo === "leitner") {
    ...
    intervals.forEach((d: number, i: number) => {
      const inp = el(
        "input",
        { type: "number", min: 1, max: 365, value: d, class: "input leitner-interval-input" },
        []
      ) as HTMLInputElement
      inp.addEventListener("change", () => {
  ...
  if (s.algo === "fsrs") {
    ...
    const setRetLabel = () => {
      retVal.textContent = Math.round(Number(retRange.value) * 100) + "%"
    }
    setRetLabel()
    ...
    algoGroup.append(
      el("div", { class: "setting-row setting-row-stack" }, [
        el("div", { class: "lab" }, [el("b", null, t("settings.algo.fsrs.measured")), measured])
      ])
    )
    void (async () => {
      try {
        ...
        measured.textContent =
          stats.reviewRetention != null
            ? t("settings.algo.fsrs.measuredFact", {
                pct: opt.formatPercent(stats.reviewRetention),
                reviews: tp("settings.algo.fsrs.reviewCount", stats.reviewCount, {
                  n: stats.reviewCount
                }),
                advice
              })
        ...
        measured.textContent = t("settings.algo.fsrs.logUnavailable")
      }
    })()

    const weightsArea = el(
      "textarea",
      { class: "input", rows: 2, placeholder: t("settings.algo.fsrs.weightsPlaceholder") },
      []
    ) as HTMLTextAreaElement
    weightsArea.value =
      Array.isArray(s.fsrsWeights) && s.fsrsWeights.length ? s.fsrsWeights.join(", ") : ""
    ...
    weightsArea.addEventListener("change", async () => {
      ...
      if (weightsArea.value.trim() && !w) {
        weightsHint.textContent = t("settings.algo.fsrs.weightsInvalid")
        return
      }
      s.fsrsWeights = w
    ...
    const exportBtn = el(
      ...
      t("settings.algo.fsrs.exportCsv")
    ) as HTMLButtonElement
    algoGroup.append(
      el("div", { class: "setting-row setting-row-stack" }, [
        el("div", { class: "lab" }, [
