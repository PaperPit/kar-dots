...
export function flashStudyCard(cardEl, isCorrect) {
  if (!cardEl) return;
...
export function showStudyFeedback(el, isCorrect, text) {
  if (!el) return;
  el.hidden = false;
...
export function pulseStudyInput(inputEl, isCorrect) {
  ...
  inputEl.addEventListener('animationend', () => inputEl.classList.remove('is-animating'), { once: true });
  if (isCorrect) inputEl.classList.add('is-correct');
  else inputEl.classList.add('is-wrong');
}

export function flashMatchPair(termEl, defEl, isCorrect, onDone) {
  const nodes = [termEl, defEl].filter(Boolean);
...
export function flashMatchHint(hintEl, isCorrect) {
  if (!hintEl) return;
