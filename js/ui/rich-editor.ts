...
export function richEditor(opts: RichEditorOpts): RichEditor {
  ...
  editable.addEventListener("click", (e: MouseEvent) => {
    if (!(e.ctrlKey || e.metaKey)) return
    const a = closestLink(e.target as Node | null)
    if (!a) return
  ...
  const highlightBtn = el(
    ...
    {
      type: "button",
      class: "rich-btn rich-btn-highlight",
  ...
  const linkBtn = el(
    "button",
    {
      ...
      onclick: (e) => {
        ...
        const sel = window.getSelection()
        if (!sel || sel.isCollapsed) {
          toast("Сначала выделите текст для ссылки", "error")
          return
        }
  ...
  const toolbar =
    opts.toolbar === false
      ? null
      : el("div", { class: "rich-toolbar" }, [boldBtn, underlineBtn, highlightWrap, linkBtn])
  const externalToolbar = !!(toolbar && opts.toolbarExternal)
  const wrap = el(
    "div",
    { class: "rich-editor" },
    externalToolbar ? [editable] : toolbar ? [toolbar, editable] : [editable]
  )
  return {
    node: wrap,
    ...
    setPlain: (text) => {
      editable.textContent = String(text || "")
    },
