...
function syncBadgeEl(badgeEl, show) {
  if (!badgeEl) return;
  badgeEl.hidden = !show;
  if (show) badgeEl.textContent = show;
}

function makeNavItems(tabs, viewName, kind) {
  return tabs.map(t => {
    const badgeEl = t.id === 'review'
      ? el('span', { class: 'badge', hidden: !t.badge }, t.badge || '')
      : null;
    const kids = kind === 'desktop'
      ? [t.label, badgeEl]
    ...
    el('div', { class: 'header-in' }, [
      brandMark({
        onclick: () => {
          ...
          const onHomeGrid = hash === '#home' || hash === '' || hash.startsWith('#folder/') || hash.startsWith('#box/');
          if (onHomeGrid && tryRavenEggClick()) return;
          nav('#home');
        },
      }),
      el('div', { class: 'header-actions' }, [
        el('nav', { class: 'nav-desktop' }, desktopNav.map(x => x.btn)),
        createThemeToggle(),
      ]),
    ])
  );

  const tabbar = el('div', { class: 'tabbar' }, tabNav.map(x => x.btn));
...
function syncShellChrome(viewName) {
  if (!shellEl) return;
  const badge = dueBadge > 0 ? String(dueBadge) : null;
...
export function setDueBadge(n) {
  dueBadge = n;
  syncShellChrome(null);
}

export async function refreshDueBadge() {
  if (!store) { dueBadge = 0; syncShellChrome(null); return 0; }
...
export function offlineBanner() {
  ...
  const statusEl = el('span', null, store.offline
    ? 'Нет сети — изменения сохранятся локально.'
    : 'Проверяю синхронизацию…');
  const actionsEl = el('div', { class: 'sync-banner-actions' });
  const banner = el('div', { class: 'offline-banner sync-banner', role: 'status' }, [statusEl, actionsEl]);
    ...
    if (pending > 0 && !store.offline) {
      actionsEl.append(el('button', {
        ...
        onclick: async () => {
          ...
          toast(r.fail ? `Синхронизировано: ${r.ok}, ошибок: ${r.fail}` : `Синхронизировано: ${r.ok}`);
          await refresh();
        },
      }, 'Повторить'));
    }
      ...
      if (letter) {
        actionsEl.append(el('button', {
          ...
          onclick: async () => {
            ...
            toast(ok ? 'Повторная синхронизация запущена' : 'Запись уже обработана', ok ? 'ok' : 'error');
            await refresh();
          },
        }, 'Повторить ошибку'));
        actionsEl.append(el('button', {
