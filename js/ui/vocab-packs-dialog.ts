import { el, toast, modal, confirmDialog, plural, spinner } from './ui.js';
...
function packInstallOverlay(modalBox, meta) {
  ...
  return {
    setDownload() {
      ...
      detailEl.textContent = plural(meta.cardCount, 'карточка', 'карточки', 'карточек') + ' · подождите';
      setPct(0);
    },
    setImport(total) {
...
export async function vocabPacksDialog() {
  let manifest;
  try {
  ...
  function renderList() {
    ...
    for (const meta of manifest.packs) {
      ...
      if (installed) {
        actions.append(
          el('button', {
            ...
            class: 'btn',
            disabled: installing,
            onclick: () => { m.close(); nav('#folder/' + installed.id); },
          }, 'Открыть'),
          el('button', {
            type: 'button',
      ...
      list.append(el('div', { class: 'vocab-pack-item' }, [
        el('div', {
        ...
        el('div', { class: 'vocab-pack-info' }, [
          ...
          el('p', { class: 'vocab-pack-meta muted' }, [
            meta.cardCount, ' ',
            plural(meta.cardCount, 'карточка', 'карточки', 'карточек'),
          ]),
        ]),
        actions,
      ]));
  ...
  async function installPack(meta) {
    ...
    const modalBox = list.closest('.modal-box');
    const overlay = packInstallOverlay(modalBox, meta);
    overlay.setDownload();

    try {
      const pack = await fetchVocabPack(meta.id);
      const total = pack.cards.filter(c => c.front?.trim()).length;
      overlay.setImport(total);
      const folder = await store.importVocabPack(pack, p => overlay.onProgress(p));
  ...
  async function removePack(meta, folder) {
  ...
  renderList();
  closeBtn.addEventListener('click', () => { if (!installing) m.close(); });

  m = modal(el('div', null, [
    el('h3', { class: 'modal-title' }, 'Лексические паки'),
    el('p', { class: 'modal-text' }, 'Готовые наборы слов по уровням CEFR. У каждого слова может быть несколько правильных переводов. Если пак уже установлен — удалите и установите заново для обновления.'),
    list,
