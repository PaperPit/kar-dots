...
export default async function handler(req) {
  ...
  const pixabayKey = cleanPixabay(payload.pixabayApiKey) || cleanPixabay(process.env.PIXABAY_API_KEY);
  const giphyKey = cleanGiphy(payload.giphyApiKey) || cleanGiphy(process.env.GIPHY_API_KEY);
