...
vi.mock('../js/ui/answer-feedback.js', () => ({
  flashMatchPair: (_a, _b, _ok, onDone) => { onDone(); },
...
describe('match mode either-side select', () => {
  ...
  it('allows selecting translation first, then term', () => {
    ...
    expect(widget.box.querySelector('.match-def[data-id="1"]').classList.contains('is-selected')).toBe(true);
  ...
  it('still allows term first, then translation', () => {
    ...
    expect(widget.box.querySelector('.match-term[data-id="2"]').classList.contains('is-selected')).toBe(true);
