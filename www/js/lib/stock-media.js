...
export function isSingleVocabWord(text) {
...
export function enrichVocabStockQuery(query) {
...
export function scoreStockRelevance(item, baseWord) {
  const word = String(baseWord || '').trim().toLowerCase();
  if (!word) return 0;
...
export function rankStockResults(items, baseWord) {
...
export function wikimediaThumbUrl(fullUrl, width = 320) {
  try {
    ...
    const encName = filename.split('/').map(p => encodeURIComponent(decodeURIComponent(p))).join('/');
...
export function pickStockThumb(row) {
  const url = String(row?.url || '');
  const wiki = wikimediaThumbUrl(url);
  if (wiki) return wiki;
  return String(row?.thumbnail || url);
}

export function normalizeOpenverseHit(row) {
  const url = String(row?.url || '');
  const thumb = pickStockThumb(row);
...
export function normalizeWikimediaHit(page, imageinfo = []) {
  const ii = imageinfo[0] || {};
  const url = String(ii.url || '');
  const title = String(page?.title || '').replace(/^File:/i, '').trim() || 'Wikimedia';
...
export async function resolveStockSearchQuery(q) {
...
export function buildStockSearchUrl({ q, type = 'photo', page = 1, pageSize = 20 }) {
...
async function searchOpenverse({ q, type, page, pageSize }) {
  ...
  return {
    ...
    items: (data.results || []).map(normalizeOpenverseHit).filter(i => i.url && i.thumb),
  };
}

async function searchWikimediaCommons({ q, limit = 12 }) {
  const params = new URLSearchParams({
    action: 'query',
    format: 'json',
    origin: '*',
  ...
  return Object.values(pages)
    .map(p => normalizeWikimediaHit(p, p.imageinfo))
    .filter(i => i.url && i.thumb && !i.isGif);
}
...
async function searchStockMediaRemote({ searchQuery, type, page, pageSize, settings }) {
  try {
    const res = await fetch('/api/stock-search', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify(withStockKeys(settings || {}, {
        q: searchQuery,
        type,
        page,
        pageSize,
...
async function searchStockMediaDirect({ searchQuery, type, page, pageSize, settings }) {
  const pixabayKey = getPixabayApiKey(settings || {});
  const giphyKey = getGiphyApiKey(settings || {});
...
async function searchStockMediaProviders({ searchQuery, type, page, pageSize, settings }) {
  const hasPixabay = (type === 'photo' || type === 'illustration') && getPixabayApiKey(settings || {});
  const hasGiphy = (type === 'gif' || type === 'sticker') && getGiphyApiKey(settings || {});
...
function openverseTypeFor(type) {
  if (type === 'sticker' || type === 'gif') return 'gif';
  return 'photo';
}

export async function searchStockMedia({ q, type = 'photo', page = 1, pageSize = 20, settings = null } = {}) {
  const raw = String(q || '').trim();
  if (!raw) {
    return {
      total: 0, page: 1, pageCount: 0, items: [],
      searchMeta: { query: '', original: '', searchQuery: '', translated: false, enriched: false, provider: 'none' },
    };
  }
  ...
  try {
    providerResult = await searchStockMediaProviders({ searchQuery, type, page, pageSize, settings });
  } catch (e) {
    return {
      total: 0,
      page: 1,
      ...
      items: [],
      searchMeta: {
        query, original, searchQuery, translated, enriched, baseWord,
        provider: 'error',
        error: e.message,
      },
    };
  ...
  const hasKeyForType = (type === 'gif' || type === 'sticker')
    ? getGiphyApiKey(settings || {})
    : getPixabayApiKey(settings || {});
...
export async function downloadStockMedia(item) {
  const res = await fetch(item.url, { mode: 'cors' });
  if (!res.ok) {
    throw new Error('Не удалось скачать изображение (CORS). Попробуйте другое или загрузите файл вручную.');
  }
  const blob = await res.blob();
  const type = blob.type || (item.isGif ? 'image/gif' : 'image/jpeg');
  const ext = item.isGif ? 'gif' : (type.includes('png') ? 'png' : 'jpg');
  const safeId = String(item.id || 'img').replace(/[^\w-]+/g, '').slice(0, 12) || 'img';
